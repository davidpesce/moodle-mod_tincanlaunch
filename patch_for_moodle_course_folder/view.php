...
//  Display the course home page.
    ...
    //ben modification 11/24/20
/**
    $course = null;
    try{
        $course = get_coursemodule_from_id('tincanlaunch', $id, 0, false, MUST_EXIST);
    } catch(Exception $e) {}

    if($course) {require_once(dirname(dirname(__FILE__)).'/mod/tincanlaunch/header.php');}
**/
    ...
    if ($completion->is_enabled()) {
        ...
        $PAGE->requires->js_init_call('M.mod_tincanlaunch.init_complete', array($id),true);
    ...
    if ($completion->is_enabled()) {
        ...
        //change by Ben Amankwata 11/24/20
        $course_has_tincan_activity = null;
        try{
            $course_has_tincan_activity = get_coursemodule_from_id('tincanlaunch', $id, 0, false, MUST_EXIST);
        } catch(Exception $e) {}

        if($course_has_tincan_activity) {
            require_once(dirname(dirname(__FILE__)).'/mod/tincanlaunch/lib.php');
            require_once(dirname(dirname(__FILE__)).'/mod/tincanlaunch/locallib.php');
